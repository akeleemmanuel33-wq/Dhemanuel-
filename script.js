...
(function () {
    ...
    function makePt() {
        return {
            ...
            col: ['#6366f1','#ec4899','#10b981','#f59e0b','#a855f7'][Math.floor(Math.random() * 5)]
        };
    }
    ...
    function drawLC() {
        ...
        pts.forEach(p => {
            ...
            if (p.y < -5 || p.x < -5 || p.x > W + 5) {
                Object.assign(p, makePt(), { y: H + 5 });
            }
        });
        ctx.globalAlpha = 1;
        if (!document.getElementById('page-loader').classList.contains('hidden')) {
            requestAnimationFrame(drawLC);
        }
    }
    drawLC();

    // Status messages cycling
    const statuses = ['Initializing', 'Loading Assets', 'Building UI', 'Almost Ready'];
    let si = 0;
    ...
    const pctInterval = setInterval(() => {
        pct = Math.min(pct + Math.floor(Math.random() * 8 + 2), 99);
